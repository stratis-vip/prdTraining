...
gp.parseFile(path.join(__dirname, "test.tcx"), (err, tcxData) => {
    if (!err) {
        ...
        let activitiesObject = interfaces_1.getActivities(resultObject); //.result//.TrainingCenterDatabase.Activities[0].Activity[0];
        ...
        console.log(`SUMMARY\nLaps: ${laps.length}
Name: ${act.name}
Type: ${enums_1.activitiesTypes[act.type]}    
LAPS`);
        laps.forEach((tcxLap) => {
            ...
            console.log(`  ${count++} from ${laps.length}
    Starting at ${lap.StartTime}
    Distance in KM:${(lap.DistanceMeters / 1000).toFixed(2)}
    TotalTime:${functions_1.secsToTime(lap.TotalTimeSeconds)}
    AverageHeartRateBpm:${lap.AverageHeartRateBpm}
    MaximumHeartRateBpm:${lap.MaximumHeartRateBpm}
    AvgSpeed:${functions_1.speedFromMpStoKpH(lap.AvgSpeed).toFixed(2)}
    MaxSpeed:${(functions_1.speedFromMpStoKpH(lap.MaximumSpeed)).toFixed(2)}
    `);
        });
        avgHR /= laps.length;
        avgCadence /= laps.length;
        console.log(`Start Time: ${act.start}
TOTALS\n\tTime in seconds: ${functions_1.secsToTime(act.totalTime)}
\tDistance in KM:  ${functions_1.distanceFromMtoKM(act.distance).toFixed(2)}
\tCalories:  ${act.calories}
\tAverage HR: ${Math.floor(avgHR)}
\tMaximum HR: ${Math.floor(maxHR)}
\tAvg Cadence : ${Math.floor(avgCadence)}
\tMaximum Speed: ${functions_1.speedFromMpStoKpH(maxSpeed).toFixed(2)}
`);
